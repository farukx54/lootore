...
import { getUserSession, clearUserSession, type UserProfile } from "@/lib/auth"
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    if (loggedInStatus) {
      ...
      // Load Ore Points from localStorage
      const storedOrePoints = localStorage.getItem("orePoints")
      if (storedOrePoints) {
        setOrePoints(Number.parseInt(storedOrePoints))
      } else {
        // If logged in but no Ore Points stored, set default
        localStorage.setItem("orePoints", orePoints.toString())
      }
    }
  }, [])
          ...
          <div className="flex items-center justify-end w-auto sm:w-[120px] md:w-[180px]">
            {isLoggedIn ? (
              <div className="flex items-center gap-1 sm:gap-2">
                ...
                <UserDropdown onLogout={handleLogout} orePoints={orePoints} userProfile={userProfile} />
              </div>
            ) : (
